...
const char* API_ENDPOINT = "http://10.80.19.101:8080/api.php"; 
...
void desenha_erro(String titulo, String mensagem) {
  ...
  display.println(F("ERRO CRITICO"));
  ...
  display.println(F("VERIFIQUE A API/REDE"));
  display.display();
  delay(100); 
}
...
void enviar_dados_lote() {
    ...
    if (httpCode > 0) {
        ...
        if (httpCode == HTTP_CODE_OK) {
            ...
            String titulo = "API Code: " + String(httpCode);
            String mensagem = http.getString().substring(0, 15) + "...";
            desenha_erro(titulo, mensagem);
            Serial.printf("ERRO de API: Servidor retornou código %d.\n", httpCode);
        }
    } else {
        String titulo = "HTTP Falhou";
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { Serial.println(F("ERRO: Falha ao iniciar o OLED.")); }
  display.display();
  delay(2000);
  display.clearDisplay(); 
  display.setTextSize(1);
  ...
  if (!LittleFS.begin(true)) { Serial.println("ERRO: Falha ao montar o LittleFS."); return; }
  Serial.println("LittleFS montado com sucesso.");
  
  // Limpeza de arquivo na reinicialização (NOVO)
  if (LittleFS.remove(DATA_FILE)) {
  ...
  if (wifi_conectado) {
    ...
    if (!sht31.begin(0x44)) { Serial.println("ERRO: Falha ao iniciar o sensor SHT31."); }
    if (!bmp.begin()) { Serial.println("ERRO: Sensor BMP180 nao encontrado!"); }
    if (!ccs.begin()) { Serial.println("ERRO: Falha ao iniciar o sensor CCS811."); while (1); }
    Serial.print("Aguardando CCS811 ficar pronto...");
    while (!ccs.available()) { delay(500); Serial.print("."); }
  ...
  if (getLocalTime(&timeinfo)) {
      ...
      if (deve_ler) {
          ...
          for (int i = 0; i < bytes_read / 2; i++) {
            ...
            if (current_amplitude > peak_amplitude) { peak_amplitude = current_amplitude; }
          }
          float noise_level_dB = 20 * log10(peak_amplitude / 32768.0) + 94;
          if (!isnan(temp_c) && !isnan(umidade)) { ccs.setEnvironmentalData(umidade, temp_c); }
          uint16_t eco2 = eco2_ultima; 
          uint16_t tvoc = tvoc_ultima; 
          bool ccs_ok = false;
          if (ccs.available()) {
            if (!ccs.readData()) { eco2 = ccs.geteCO2(); tvoc = ccs.getTVOC(); ccs_ok = true; } 
            else { Serial.println("ERRO na leitura do CCS811!"); }
          }
          if (!isnan(temp_c) && !isnan(umidade)) { temp_c_ultima = temp_c; umidade_ultima = umidade; }
          pressaoHPa_ultima = pressaoHPa; 
          if (ccs_ok) { eco2_ultima = eco2; tvoc_ultima = tvoc; }
          ...
          pagina_atual++;
          if (pagina_atual > 2) { pagina_atual = 1; }
      }
  }

  if (digitalRead(LED_RED) == LOW) {
    if (pagina_atual == 1) { desenha_pagina_1(); } 
    else { desenha_pagina_2(); }
  }
